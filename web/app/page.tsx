...
function BuyCrypto() {
  ...
  const rates: Record<string, number> = { ETH: 3420.18, BTC: 68420.12, USDC: 1 }
  const received = fiatAmount / rates[asset]
  const fee = fiatAmount * 0.0125
  const [checkoutState, setCheckoutState] = useState<"idle" | "loading" | "success" | "error">("idle")

  async function handleCheckout() {
    setCheckoutState("loading")
    try {
      const response = await fetch("/api/onramp/session", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ asset, amountUsd: fiatAmount, network, paymentMethod: payment, secureCheckout: secure }) })
  ...
  return <div className="mx-auto max-w-[1180px] px-5 py-8 md:px-10 md:py-10">
    <div className="mb-8"><p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary">Buy crypto</p><h2 className="text-3xl font-semibold tracking-tight md:text-4xl">Fund your portfolio</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Purchase digital assets securely with your preferred payment method.</p></div>
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <form className="rounded-2xl border border-border bg-card p-5 md:p-7" onSubmit={(event) => event.preventDefault()}>
        <div className="flex items-center justify-between border-b border-border pb-5"><div><h3 className="font-semibold">Purchase details</h3><p className="mt-1 text-sm text-muted-foreground">Choose what you want to add</p></div><div className="grid size-10 place-items-center rounded-xl bg-primary/15 text-primary"><Wallet className="size-5" /></div></div>
        <div className="mt-7 flex flex-col gap-6">
          <label className="flex flex-col gap-2"><span className="text-sm font-medium">Asset</span><div className="relative"><select aria-label="Select asset" value={asset} onChange={(event) => setAsset(event.target.value)} className="w-full appearance-none rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none transition focus:border-primary"><option value="ETH">Ethereum (ETH)</option><option value="BTC">Bitcoin (BTC)</option><option value="USDC">USD Coin (USDC)</option></select><ChevronDown className="pointer-events-none absolute right-4 top-3.5 size-4 text-muted-foreground" /></div></label>
          <label className="flex flex-col gap-2"><span className="text-sm font-medium">Amount</span><div className="flex items-center rounded-xl border border-border bg-background px-4 focus-within:border-primary"><span className="text-lg text-muted-foreground">$</span><input aria-label="Amount in USD" inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value.replace(/[^0-9.]/g, ""))} className="min-w-0 flex-1 bg-transparent px-2 py-3 text-lg font-semibold outline-none" /><span className="text-xs font-medium text-muted-foreground">USD</span></div><span className="text-xs text-muted-foreground">Minimum purchase $10.00</span></label>
          <div className="grid gap-6 sm:grid-cols-2"><label className="flex flex-col gap-2"><span className="text-sm font-medium">Payment method</span><div className="relative"><select aria-label="Select payment method" value={payment} onChange={(event) => setPayment(event.target.value)} className="w-full appearance-none rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary"><option>Visa •••• 4242</option><option>Apple Pay</option><option>Bank transfer</option></select><ChevronDown className="pointer-events-none absolute right-4 top-3.5 size-4 text-muted-foreground" /></div></label><label className="flex flex-col gap-2"><span className="text-sm font-medium">Network</span><div className="relative"><select aria-label="Select network" value={network} onChange={(event) => setNetwork(event.target.value)} className="w-full appearance-none rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary"><option>Ethereum</option><option>Arbitrum</option><option>Base</option></select><ChevronDown className="pointer-events-none absolute right-4 top-3.5 size-4 text-muted-foreground" /></div></label></div>
          <button type="button" role="switch" aria-checked={secure} onClick={() => setSecure(!secure)} className="flex items-center justify-between rounded-xl border border-border bg-background p-4 text-left"><div className="flex items-center gap-3"><div className={`grid size-9 place-items-center rounded-lg ${secure ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}`}><ShieldCheck className="size-4" /></div><div><p className="text-sm font-medium">Use Secure Checkout</p><p className="mt-1 text-xs text-muted-foreground">Extra verification for every purchase</p></div></div><span className={`relative h-6 w-11 rounded-full transition ${secure ? "bg-primary" : "bg-muted"}`}><span className={`absolute top-1 size-4 rounded-full bg-primary-foreground transition ${secure ? "left-6" : "left-1"}`} /></span></button>
        </div>
      </form>
      <aside className="h-fit rounded-2xl border border-primary/25 bg-primary/5 p-5 md:p-7"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Order summary</p><div className="mt-6 flex items-center justify-between"><span className="text-sm text-muted-foreground">You pay</span><span className="text-lg font-semibold">${fiatAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div><div className="mt-4 flex items-center justify-between"><span className="text-sm text-muted-foreground">Estimated receive</span><span className="text-sm font-semibold">{received.toFixed(asset === "USDC" ? 2 : 6)} {asset}</span></div><div className="mt-4 flex items-center justify-between"><span className="text-sm text-muted-foreground">Network fee</span><span className="text-sm font-medium">${fee.toFixed(2)}</span></div><div className="my-6 border-t border-border" /><div className="flex items-start gap-3 text-xs leading-5 text-muted-foreground"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-success" />Your transaction is protected by encrypted checkout and network confirmation.</div><button type="button" onClick={handleCheckout} disabled={checkoutState === "loading"} className="mt-7 flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3.5 text-sm font-semibold text-primary-foreground shadow-[0_8px_24px_hsl(var(--primary)/.2)] transition hover:brightness-110 disabled:cursor-wait disabled:opacity-70">{checkoutState === "loading" ? "Creating secure session…" : checkoutState === "success" ? "Checkout session ready" : "Continue to checkout"} <ArrowDownLeft className="size-4 rotate-[-45deg]" /></button><p className={`mt-4 text-center text-[11px] ${checkoutState === "error" ? "text-destructive" : checkoutState === "success" ? "text-success" : "text-muted-foreground"}`}>{checkoutState === "error" ? "Unable to reach the gateway. Try again." : checkoutState === "success" ? "Your secure session was created." : "Rate locked for 30 seconds"}</p></aside>
    </div>
  </div>
}
